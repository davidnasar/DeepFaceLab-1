                ...
                if len(data_rects) == 0:
                    ...
                    if self.cache_text_lines_img[0] == sh:
                        ...
                        self.text_lines_img = (imagelib.get_draw_text_lines ( self.image, sh,
                                                        [   '点击鼠标[Mouse click] - 加锁解锁lock/unlock selection',
                                                            '滚动鼠标[Mouse wheel] - 改变选区change rect',
                                                            '回车[Enter] / 空格[Space] - 确认confirm / 跳过skip frame',
                                                            '[,] [.]- 上一帧prev frame, 下一帧next frame. [Q] - 跳过skip remaining frames',
                                                            '[a] - accuracy on/off (more fps)',
                                                            '[h] - 隐藏帮助 hide this help'
...
def main(input_dir,
    ...
    if output_path.exists():
        if not manual_output_debug_fix and input_path != output_path:
            output_images_paths = Path_utils.get_image_paths(output_path)
            if len(output_images_paths) > 0:
                io.input_bool("警告 !!! \n %s 已经有图片了!  \n 按回车[Enter]继续 \n 继续执行会删除之前的图片 \n 不想覆盖右上角关闭" % (str(output_path)), False )
                for filename in output_images_paths:
                    Path(filename).unlink()
    else:
        output_path.mkdir(parents=True, exist_ok=True)
