...
class Model(ModelBase):
    ...
    def onInitializeOptions(self, is_first_run, ask_override):
        if is_first_run:
            self.options['lighter_ae'] = io.input_bool ("启用轻量级[Use lightweight autoencoder]? (y/n, ?:help skip:n) : ", False, help_message="轻量级自动编码器速度更快，需要的VRAM更少，牺牲了整体质量。 如果您的GPU VRAM <= 4，则应选择此选项。")
        else:
