...
class SAEModel(ModelBase):
    ...
    def onInitializeOptions(self, is_first_run, ask_override):
        ...
        if is_first_run:
            resolution = io.input_int("像素[Resolution=]( 64-256 ?:help skip:128) : ", default_resolution, help_message="更高的分辨率需要更多的VRAM和训练时间。取值为16的倍数.")
            resolution = np.clip (resolution, 64, 256)
            while np.modf(resolution / 16)[0] != 0.0:
                resolution -= 1
            self.options['resolution'] = resolution

            self.options['face_type'] = io.input_str ("全脸还是半脸[Half or Full face]? (h/f, ?:help skip:f) : ", default_face_type, ['h','f'], help_message="Half face has better resolution, but covers less area of cheeks.").lower()
        ...
        if (is_first_run or ask_override) and 'tensorflow' in self.device_config.backend:
            ...
            self.options['optimizer_mode'] = io.input_int ("优化模式[Optimizer mode]? ( 1,2,3 ?:help skip:%d) : " % (def_optimizer_mode), def_optimizer_mode, help_message="1 - no changes. 2 - allows you to train x2 bigger network consuming RAM. 3 - allows you to train x3 bigger network consuming huge amount of RAM and slower, depends on CPU power.")
        else:
            self.options['optimizer_mode'] = self.options.get('optimizer_mode', 1)

        if is_first_run:
            self.options['archi'] = io.input_str ("模型结构[AE architecture] (df, liae ?:help skip:%s) : " % (default_archi) , default_archi, ['df','liae'], help_message="'df' keeps faces more natural. 'liae' can fix overly different face shapes.").lower() #-s version is slower, but has decreased change to collapse.
        else:
        ...
        if is_first_run:
            self.options['ae_dims'] = np.clip ( io.input_int("自动编码器维度[AutoEncoder dims] (32-1024 ?:help skip:%d) : " % (default_ae_dims) , default_ae_dims, help_message="All face information will packed to AE dims. If amount of AE dims are not enough, then for example closed eyes will not be recognized. More dims are better, but require more VRAM. You can fine-tune model size to fit your GPU." ), 32, 1024 )
            self.options['e_ch_dims'] = np.clip ( io.input_int("每个通道编码器维度 [Encoder dims per channel] (21-85 ?:help skip:%d) : " % (default_e_ch_dims) , default_e_ch_dims, help_message="More encoder dims help to recognize more facial features, but require more VRAM. You can fine-tune model size to fit your GPU." ), 21, 85 )
            default_d_ch_dims = self.options['e_ch_dims'] // 2
            self.options['d_ch_dims'] = np.clip ( io.input_int("每个通道解码器维度 [Decoder dims per channel] (10-85 ?:help skip:%d) : " % (default_d_ch_dims) , default_d_ch_dims, help_message="More decoder dims help to get better details, but require more VRAM. You can fine-tune model size to fit your GPU." ), 10, 85 )
            self.options['multiscale_decoder'] = io.input_bool ("使用多尺度解码器 [Use multiscale decoder]? (y/n, ?:help skip:n) : ", False, help_message="Multiscale decoder helps to get better details.")
            self.options['ca_weights'] = io.input_bool ("使用CA权重[Use CA weights]? (y/n, ?:help skip: %s ) : " % (yn_str[def_ca_weights]), def_ca_weights, help_message="Initialize network with 'Convolution Aware' weights. This may help to achieve a higher accuracy model, but consumes a time at first run.")
        else:
        ...
        if is_first_run or ask_override:
            ...
            self.options['pixel_loss'] = io.input_bool ("使用像素丢失[pixel loss]? (y/n, ?:help skip: %s ) : " % (yn_str[def_pixel_loss]), def_pixel_loss, help_message="像素丢失可能有助于增强细节和稳定面部颜色。 仅在质量不随时间改善时使用。")

            default_face_style_power = default_face_style_power if is_first_run else self.options.get('face_style_power', default_face_style_power)
            self.options['face_style_power'] = np.clip ( io.input_number("脸部风格强度[Face style power] ( 0.0 .. 100.0 ?:help skip:%.2f) : " % (default_face_style_power), default_face_style_power,
            ...
            self.options['bg_style_power'] = np.clip ( io.input_number("背景风格强度[Background style power] ( 0.0 .. 100.0 ?:help skip:%.2f) : " % (default_bg_style_power), default_bg_style_power,
            ...
            if nnlib.device.backend != 'plaidML': # todo https://github.com/plaidml/plaidml/issues/301
                ...
                self.options['clipgrad'] = io.input_bool ("启用渐变剪裁[Enable gradient clipping]? (y/n, ?:help skip:%s) : " % (yn_str[default_clipgrad]), default_clipgrad, help_message="Gradient clipping reduces chance of model collapse, sacrificing speed of training.")
            else:
